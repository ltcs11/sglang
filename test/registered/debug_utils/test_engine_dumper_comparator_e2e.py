...
class TestSourcePatcherE2ESGLang:
    ...
    def test_dp_attention(self, tmp_path: Path) -> None:
        ...
        Attention tensors (attn_output, attn_pre_o_proj) are allowed to
        fail/skip because dp-attention causes empty-batch ranks to skip
        attention dumps entirely (the ``if hidden_states.shape[0] != 0``
        guard), producing asymmetric field sets.  Post-attention tensors
        (pre_mlp_residual, moe_router_logits, moe_expert_output,
        mlp_output) are still compared and must pass.
        """
        _run_e2e_scenario(
            tmp_path=tmp_path,
            ...
            extra_comparator_args=[
                ...
                "input_ids|positions|attn_output|attn_pre_o_proj",
                "--allow-failed-pattern",
                "attn_output|attn_pre_o_proj|layer_input",
            ],
        )
